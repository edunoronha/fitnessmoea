...
public class SolutionSet implements Serializable {
    ...
    public void printObjectivesToFile(String path, String path2) throws WriteException, BiffException {
        try {
            ...
            WritableSheet sheet = workbook.createSheet("objetivos", 0);
            ...
            File a = new File((directory[0] + "/" + directory[1]));

            a.mkdirs();

            printObjectivesToFile(path, path2);
        }
    } // printObjectivesToFile

    public void printPromToFile(String path, double[][] prom) throws WriteException, BiffException {
        try {
            /* Open the file */
            ...
            for (int i = 0; i < prom.length; i++) {
                String linea = "";
                for (int j = 0; j < prom[i].length; j++) {
                    jxl.write.Number objetive = new jxl.write.Number(j + 3, i + 1, prom[i][j]);
                    sheet.addCell(objetive);
                    linea += prom[i][j] + " ";
                }
                bw.write(linea);
    ...
    public void printTimeToFile(String path, long[] time) throws WriteException, BiffException {
        try {
            /* Open the file */
    ...
    public void printVariablesToFile(String path, String path2) throws BiffException, WriteException {
        try {
            ...
            printVariablesToFile(path, path2);
